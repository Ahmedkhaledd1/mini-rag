...
async def process_endpoint(request: Request,project_id: str,process_request: ProcessRequest):
    ...
    project =await project_model.get_project_or_create(project_id=project_id)
    chunk_model=await ChunkModel.create_insatnce(db_client=request.app.db_client)
    ...
    if process_request.file_name:
        ...
        if asset_record is None:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"status":f"File {process_request.file_name} not found in project {project_id}"}
            )
        ...
        project_assets=await asset_model.get_all_assets_project(
            asset_project_id=project.id,
            asset_type=AssetTypeEnum.FILE.value
        )
        if len(project_assets)==0:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"status":"No files found in the project to process"}
            )
        project_files_ids={
    ...
    for asset_id,file_name in project_files_ids.items():
        file_content=process_controller.get_file_content(filename=file_name)
    ...
    return JSONResponse(
        ...
        content={"status":ResponseStatus.PROCESSING_SUCCESS.value,
                ...
                "total_files_processed":no_of_files}
    )
